...
reload(mh)
import getpass
import spider_tree_structure as s
...
def delete(pulseNo, node):
    t = Tree('SPIDER', pulseNo, 'edit')

    # get the username of who wrote this run
    try:
        n = t.getNode(r'\SPIDER::TOP.' + node + '.CODE_VERSION:USER')
...
def create(pulseNo, node, descr):#,machin):
    ...
    descriptions = [descr]
    spider = t.getDefault()
    s.tree(t,branches,descriptions,spider)#,machin)
